    ...
    >
      <Box sx={{ p: 2 }}>
        ...
        <FilterPanel 
          ...
          gdpRange={[minGdpFilter, maxGdpFilter]}
          onGdpChange={(range: [number, number]) => {
            setMinGdpFilter(range[0]);
            setMaxGdpFilter(range[1]);
          }}
          maxGdp={d3.max(processedData, d => d.gdp_per_capita)!}
        />
      </Box>
      <svg ref={svgRef}></svg>
      <Tooltip 
